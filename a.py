...
pygame.init()
snake_img = pygame.image.load("head_up.png")
...
def draw_grass():
    for i in range(HEIGHT):
        for j in range(WIDTH):
            if (i + j) % 2 == 0:
                pygame.draw.rect(dis, green, [SNAKE_BLOCK * j, SNAKE_BLOCK * i, SNAKE_BLOCK, SNAKE_BLOCK])
            else:
                pygame.draw.rect(dis, yellow, [SNAKE_BLOCK * j, SNAKE_BLOCK * i, SNAKE_BLOCK, SNAKE_BLOCK])
def initial_game():
